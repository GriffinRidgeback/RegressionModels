...
Having fit several models, which of those models provides the best fit and, subsequently, the best predictor?  To answer that question, the **$R^{2}$** (coefficient of determination) statistics for all three models are provided in **Table 1**.  With the exception of the first entry, all 3 models explain between 77% and 87% of the total variation in MPG, with the interactive model being the best estimator.  However, an examination of the coefficient p-values for all three models resulted in a determination that only formula _mpg ~ wt_ had any signficant regressors ($p_A$ = 0.0001246 and $p_M$ = 0.0000169).

The findings of the graphs are supported by analysis of the model coefficients for the various selected regressors.  In all cases, the coefficients for weight are negative, indicating an inverse relationship between weight and fuel economy (see _Tables 2 - 4_).  So, an increase in 1000 lbs of weight will have a negative $\beta_1$ influence.  The effects are mitigated somewhat by including an additional regressor and even more so when fiting a model with interaction terms.  However, in all cases, the decrease in MPG is more severely seen in the case of vehicles with manual transmissions.

# Residuals and Diagnostics
A plot of the residuals, shown in **Figure 2**, is a graphical, rather than tabular, evaluation of model fit.  From the plot of variations in MPG by car weight, there are three distinct patterns, two at either end and the more significant central cluster.  The randomness of these patterns is reinforced by the second plot of residuals against weight.  The data is fairly symmetrical, indicating that **$\mathbf{E[e_i]}$ = 0**
...
plot(x, y, xlab = "Weight", ylab = "MPG", bg = "lightblue",col = "black", cex = 2, pch = 21,frame = FALSE,main = "Residual on Regression Line")
# draw linear regression line
abline(fit, lwd = 2)
# draw red lines from data points to regression line
for (i in 1 : n)
        {
        lines(c(x[i], x[i]),c(y[i], yhat[i]), col = "orange" , lwd = 2)
        }
# plot residual vs x
plot(x, e, xlab = "Weight", ylab = "MPG", bg = "lightblue",col = "black", cex = 2, pch = 21,frame = FALSE,main = "Residual vs X")
# draw horizontal line
abline(h = 0, lwd = 2)
# draw red lines from residual to x axis
for (i in 1 : n)
        {
        lines(c(x[i], x[i]),c(e[i], 0), col = "orange" , lwd = 2)
        }
```
...
**_Table 2_**: Coefficients regressed by weight
...
**_Table 3_**: Coefficients regressed by weight and horsepower
...
**_Table 4_**: Coefficients regressed by weight and horsepower, interactively
